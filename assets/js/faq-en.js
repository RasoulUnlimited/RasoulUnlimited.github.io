document.addEventListener('DOMContentLoaded', () => {
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', () => {
            ...
            if (typeof hj === 'function') {
                hj('event', `faq_nav_clicked_category_${category.replace(/\s/g, '_').toLowerCase()}`);
            }
        });
    });
    ...
    searchInput.addEventListener('input', () => {
        ...
        allSections.forEach(section => {
            ...
            itemsInSection.forEach(item => {
                ...
                const answerText = answerTextElement ? answerTextElement.textContent.toLowerCase() : '';
                const keywords = item.dataset.keywords ? item.dataset.keywords.toLowerCase() : '';

                if (questionText.includes(searchTerm) || answerText.includes(searchTerm) || keywords.includes(searchTerm)) {
        ...
        if (typeof hj === 'function') {
            hj('event', `faq_searched_${searchTerm.replace(/\s/g, '_').toLowerCase()}`);
        }
    });
    ...
    faqItems.forEach(item => {
        ...
        if (answer) { // Ensure answer element exists
            ...
            answer.style.transition = 'max-height 0.4s var(--ease-bezier), padding 0.4s var(--ease-bezier), opacity 0.4s ease-out'; 
        ...
        summary.addEventListener('click', (event) => {
            ...
            faqItems.forEach(otherItem => {
                ...
                if (otherItem !== item && otherItem.open && !otherItem.hasAttribute('data-opened-by-search')) {
                    otherItem.open = false;
                    otherItem.removeAttribute('data-opened-by-user'); // Remove the flag
                    const otherAnswer = otherItem.querySelector('.faq-answer');
                    if (otherAnswer) { // Ensure otherAnswer exists
                    ...
                    if (typeof gtag === 'function') {
                        gtag('event', 'faq_auto_collapse', {
                            ...
                            'question_text': otherItem.querySelector('summary').textContent.trim()
                        });
                    }
                    if (typeof hj === 'function') {
            ...
            if (wasAlreadyOpen) { // If it was open and user clicked to close it
                if (answer) {
                    answer.style.maxHeight = '0px';
                    answer.style.paddingTop = '0';
                    answer.style.paddingBottom = '0';
            ...
            } else { // If it was closed and user clicked to open it
        ...
        summary.addEventListener('focus', () => {
            summary.style.outline = `3px solid ${getComputedStyle(document.documentElement).getPropertyValue('--primary-blue')}66`; 
